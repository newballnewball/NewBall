...
export default function WinnerModal({ winner, pot, tier, onClose, currentUserId }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:24,backdropFilter:"blur(10px)" }}>
      ...
      <div style={{ background:"#0d1018",border:"1px solid rgba(251,191,36,0.3)",borderRadius:24,padding:"32px 24px",maxWidth:380,width:"100%",textAlign:"center",position:"relative",overflow:"hidden",animation:"winner-glow 2s ease infinite",fontFamily:"'DM Sans',sans-serif" }}>
        <div style={{ position:"absolute",top:-50,left:"50%",transform:"translateX(-50%)",width:220,height:220,borderRadius:"50%",background:"radial-gradient(circle,rgba(251,191,36,0.2),transparent 70%)",pointerEvents:"none" }}/>

        <div style={{ marginBottom:16,animation:"winner-pop 0.6s cubic-bezier(0.34,1.56,0.64,1)" }}>
          <Nooball size={76} spin="fast"/>
        </div>

        <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"rgba(251,191,36,0.6)",letterSpacing:"0.18em",marginBottom:6,animation:"slide-up 0.4s ease 0.3s both" }}>
          {tier?.label?.toUpperCase() || "WEEKLY"} WINNER
        </div>

        {isWinner ? (
          <>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:42,color:"#fbbf24",letterSpacing:"0.04em",lineHeight:1,marginBottom:6,animation:"slide-up 0.4s ease 0.4s both" }}>
              THAT'S YOU!
            </div>
        ...
        <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:54,background:"linear-gradient(135deg,#fbbf24,#f97316)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",letterSpacing:"0.04em",marginBottom:20,animation:"slide-up 0.4s ease 0.5s both" }}>
          ${pot}
        </div>
        ...
        <div style={{ background:"rgba(251,191,36,0.07)",border:"1px solid rgba(251,191,36,0.18)",borderRadius:12,padding:"14px 16px",marginBottom:18,animation:"slide-up 0.4s ease 0.6s both" }}>
          {winner?.venmoHandle
            ?<><div style={{ fontSize:13,color:"rgba(255,255,255,0.45)",marginBottom:4 }}>Send ${tier?.amount || "1"} to</div>
               <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:"#fbbf24",letterSpacing:"0.05em",marginBottom:8 }}>@{winner.venmoHandle}</div></>
