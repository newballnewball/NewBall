...
export default function LandingScreen({ onJoin }) {
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"#0a0a0f", minHeight:"100vh", maxWidth:430, margin:"0 auto", color:"#fff", display:"flex", flexDirection:"column" }}>
      ...
      {/* ── Hero ─────────────────────────────────────────── */}
      <div style={{ background:"linear-gradient(170deg,#110828 0%,#0a0a0f 100%)", padding:"60px 24px 40px", textAlign:"center", position:"relative", overflow:"hidden", flex:1, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:8, marginBottom:8, animation:"fade-up 0.4s ease 0.2s both" }}>
          {[["d","days"],["h","hrs"],["m","min"],["s","sec"]].map(([k,l])=>(
            <div key={k} style={{ textAlign:"center" }}>
              <div style={{ background:"rgba(255,255,255,0.06)", borderRadius:8, padding:"6px 10px", fontFamily:"'DM Mono',monospace", fontSize:18, fontWeight:500, color:"#fff", lineHeight:1, minWidth:38 }}>{pad(cd[k])}</div>
        ...
        {/* Member count */}
        {memberCount > 0 && (
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(167,139,250,0.08)", border:"1px solid rgba(167,139,250,0.18)", borderRadius:24, padding:"7px 16px", marginBottom:28, animation:"fade-up 0.4s ease 0.25s both" }}>
            ...
            <span style={{ fontSize:13, color:"rgba(255,255,255,0.65)" }}>
              <strong style={{ color:"#fff", fontFamily:"'Bebas Neue',sans-serif", fontSize:15, letterSpacing:"0.04em" }}>{memberCount}</strong> {memberCount === 1 ? "person" : "people"} already in
            </span>
          </div>
        )}
        ...
        <div style={{ animation:"fade-up 0.4s ease 0.3s both" }}>
          <button className="join-btn" onClick={onJoin} style={{ background:"linear-gradient(135deg,#7c3aed,#a855f7)", border:"none", borderRadius:18, color:"#fff", fontSize:26, padding:"22px 72px", cursor:"pointer", fontFamily:"'Bebas Neue',sans-serif", letterSpacing:"0.1em", animation:"btn-pulse 2.5s ease infinite", boxShadow:"0 4px 30px rgba(124,58,237,0.4)" }}>
            I'M IN — $1
          </button>
        </div>
      ...
      <div style={{ padding:"24px 24px 48px" }}>
        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, color:"rgba(255,255,255,0.28)", letterSpacing:"0.15em", marginBottom:14 }}>WHAT PEOPLE WANT</div>
        <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:22 }}>
          {shownWishes.map((w,i)=>(
            <div key={i} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:12, padding:"12px 14px", display:"flex", gap:10, alignItems:"center" }}>
              <div style={{ width:32,height:32,borderRadius:"50%",background:`hsl(${(i*80+260)%360},50%,55%)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"#fff",flexShrink:0 }}>
