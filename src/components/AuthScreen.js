...
async function venmoTaken(handle) {
  const clean = handle.replace("@","").toLowerCase();
  const q = query(collection(db,"users"), where("venmoHandle","==",clean));
...
const inp = { width:"100%", padding:"13px 16px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, color:"#fff", fontSize:14, fontFamily:"inherit", outline:"none" };
...
export default function AuthScreen({ invitedBy, onBack }) {
  ...
  const checkVenmo = async (handle) => {
    if (!handle) return true; // optional for login
    const clean = handle.replace("@","").toLowerCase();
    if (await venmoTaken(clean)) throw new Error("That Venmo handle is already registered. Each handle can only be used once.");
    return clean;
  };
  ...
  const loginGoogle = () => attempt(async () => {
    ...
    if (!snap.exists()) {
      // new Google user — need Venmo
      // will be prompted after
  ...
  const loginEmail = () => attempt(async () => {
    ...
    if (mode === "signup") {
      ...
      const cleanVenmo = await checkVenmo(venmo);
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:24, background:"#0a0a0f", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ width:"100%", maxWidth:360, animation:"fade-up 0.4s ease", fontFamily:"'DM Sans',sans-serif" }}>
        {onBack && (
          <button onClick={onBack} style={{ background:"transparent",border:"none",color:"rgba(255,255,255,0.3)",cursor:"pointer",fontSize:13,padding:0,marginBottom:20,fontFamily:"inherit" }}>
            ← Back
          </button>
        )}

        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{ marginBottom:14 }}><Baseball size={64} spin="fast" /></div>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
          ...
          <input type="password" placeholder="Password" value={password} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&loginEmail()} style={inp}/>
          {mode === "signup" && (
            <div>
              <input placeholder="Venmo handle (optional — e.g. @yourname)" value={venmo} onChange={e=>setVenmo(e.target.value)} style={inp}/>
              <div style={{ fontSize:11,color:"rgba(255,255,255,0.25)",marginTop:5,paddingLeft:4 }}>Used to receive your winnings. You can add this later.</div>
            </div>
          )}
          {error && <div style={{ fontSize:12,color:"#f87171",textAlign:"center",padding:"4px 0" }}>{error}</div>}
          <button onClick={loginEmail} disabled={loading} style={{ width:"100%",padding:14,background:"linear-gradient(135deg,#7c3aed,#ec4899)",border:"none",borderRadius:12,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:4,animation:"glow 3s ease infinite",transition:"transform 0.15s" }}>
